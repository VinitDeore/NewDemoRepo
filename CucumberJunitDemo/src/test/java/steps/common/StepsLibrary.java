...
public class StepsLibrary {
	...
	public static void close(String loc) {
		...
		try {
			...
			for (int i = 0; i < windowStrings.size(); i++) {
				if (windowStrings.get(i).equalsIgnoreCase(parentHandle)) {
					System.out.println("parentHandle ::  " + parentHandle);
					new WebDriverTestBase().getDriver().close();
				}
			}
			for (int i = 0; i < windowStrings.size(); i++) {
				new WebDriverTestBase().getDriver().switchTo().window(windowStrings.get(windowStrings.size() - 1));
			}
		} catch (Exception e) {
			Set<String> newWindowHandles = new WebDriverTestBase().getDriver().getWindowHandles();
			List<String> newWindowStrings = new ArrayList<>(newWindowHandles);
			for (int i = 0; i < newWindowStrings.size(); i++) {
				new WebDriverTestBase().getDriver().switchTo().window(newWindowStrings.get(newWindowStrings.size() - 1));
			}
		}
	}
	...
	@QAFTestStep(description = "clickAndSendKeys {text} into {loc}")
	...
	@QAFTestStep(description = "clickAndSendEncryptedKeys {text} into {loc}")
